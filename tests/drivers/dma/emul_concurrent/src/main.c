...
ZTEST(dma_emul_concurrent, test_start_chain_rejects_active_downstream)
{
	...
	for (int i = 0; i < NUM_CHANNELS; i++) {
		...
		k_sem_init(&ctx[i].sem, 0, 1);
		for (int j = 0; j < XFER_SIZE; j++) {
			ctx[i].tx[j] = (uint8_t)(j + 0x40 * i);
		}
		block_cfg[i].block_size = XFER_SIZE;
#ifdef CONFIG_DMA_64BIT
		block_cfg[i].source_address = (uint64_t)(uintptr_t)ctx[i].tx;
		block_cfg[i].dest_address = (uint64_t)(uintptr_t)ctx[i].rx;
#else
		block_cfg[i].source_address = (uint32_t)(uintptr_t)ctx[i].tx;
		block_cfg[i].dest_address = (uint32_t)(uintptr_t)ctx[i].rx;
#endif
		dma_cfg[i].channel_direction = MEMORY_TO_MEMORY;
		dma_cfg[i].source_data_size = 1U;
		dma_cfg[i].dest_data_size = 1U;
