...
bridge (UART0). If the serial port is not auto-detected, pass it
explicitly::

   west flash --runner esp32 --esp-device /dev/ttyACM0

The Zephyr console is the USB Serial/JTAG interface, which shows up as a
separate USB CDC ACM device. Open a serial terminal on that port at
115200 bps::
